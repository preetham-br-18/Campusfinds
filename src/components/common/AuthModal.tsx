...
import { X, Mail, Lock, User, AlertCircle, CheckCircle, GraduationCap, Building2, ShieldCheck, Sparkles } from 'lucide-react';
...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const { loginWithEmail, registerWithEmail, loginWithGoogle, quickSignInAsRole } = useAuth();
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (msg.includes('auth/unauthorized-domain')) {
        msg = 'Preview domain is not registered in Firebase Authorized Domains. Use the 1-Tap Instant Sign-In below.';
  ...
  const handleQuickSignIn = async (role: 'admin' | 'student') => {
    setError(null);
    setLoading(true);
    try {
      await quickSignInAsRole(role);
      onClose();
    } catch (err: any) {
      console.warn('Quick sign-in notice:', err);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-150">
      <div className="relative w-full max-w-md bg-white dark:bg-slate-900 rounded-t-3xl sm:rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 p-6 overflow-hidden max-h-[90vh] overflow-y-auto">
        ...
        {/* 1-Tap Instant Campus Sign-In */}
        <div className="mt-3 p-3 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200/70 dark:border-slate-700/60">
          <div className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 mb-2 flex items-center justify-between">
            ...
            <span className="text-[10px] text-indigo-600 dark:text-indigo-400 font-bold uppercase tracking-wider">Fast Access</span>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <button
              type="button"
              ...
              disabled={loading}
              className="px-2.5 py-2 text-xs font-semibold rounded-lg bg-indigo-50 dark:bg-indigo-950/50 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800 hover:bg-indigo-100 dark:hover:bg-indigo-900/60 transition-colors flex items-center justify-center space-x-1.5 touch-manipulation min-h-[38px] active:scale-[0.98]"
            >
              <ShieldCheck className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400 shrink-0" />
              <span className="truncate">Admin (Prajju)</span>
            </button>
            <button
              type="button"
              id="quick-signin-student-btn"
              onClick={() => handleQuickSignIn('student')}
              disabled={loading}
              className="px-2.5 py-2 text-xs font-semibold rounded-lg bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800 hover:bg-emerald-100 dark:hover:bg-emerald-900/60 transition-colors flex items-center justify-center space-x-1.5 touch-manipulation min-h-[38px] active:scale-[0.98]"
            >
              <GraduationCap className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400 shrink-0" />
              <span className="truncate">Student (Ananya)</span>
            </button>
          </div>
        </div>
