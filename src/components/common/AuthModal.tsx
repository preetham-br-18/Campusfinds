    ...
    try {
      if (mode === 'login') {
        ...
        await registerWithEmail({
          ...
          college: college.trim() || 'Campus University',
          studentId: studentId.trim() || 'ID-' + Math.floor(100000 + Math.random() * 900000)
        });
      }
      onClose();
    } catch (err: any) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (msg.includes('auth/unauthorized-domain')) {
        msg = 'Google Sign-In is unavailable on this preview domain. Please sign in or register with your campus email below.';
